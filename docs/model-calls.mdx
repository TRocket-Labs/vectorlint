...
VectorLint supports three review approaches through `--model-call`. Leave the default, `auto`, enabled for most reviews. Override it when the size or structure of a particular document calls for a different approach.

## Choose an approach

| Value | Use it for |
| --- | --- |
| `auto` | Most reviews. VectorLint selects an approach for the current document. |
