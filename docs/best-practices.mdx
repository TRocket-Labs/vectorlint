...
Start with `VECTORLINT.md`. It requires no configuration, activates immediately, and brings your most important standards in plain language before you commit to structured rules.
...
VectorLint emits a warning at 4,000 tokens, but a practical target is *much* lower. Under 800 tokens leaves headroom for rule-specific instructions to add context without the combined system context becoming difficult to process. Long context degrades LLM precision and increases API costs on every review.
...
## Write specific rules, not general ones

The quality of VectorLint's findings is directly proportional to the specificity of your rules. Vague rules produce vague findings — or worse, inconsistent findings that vary by run.
...
The second rule gives the LLM a defined audience, measurable criteria, and specific examples. It will produce consistent, actionable findings.

## Give rules domain context

LLMs review content relative to an implied standard. Make that standard explicit with a context block in your rule. Tell the model who the audience is, what they value, and what good looks like for your specific content type.
...
4. Raise strictness on your highest-stakes rules first
...
- [Handling false positives](/handling-false-positives) — detailed guidance on CONFIDENCE_THRESHOLD and strictness
- [CI Integration](/ci-integration) — set up content quality gates in your pipeline
- [Customize style rules](/customize-style-rules) — write effective rules for rule pack files
