...
| `Concurrency` | `4` | Number of rule reviews to run in parallel. Reduce this if you hit API rate limits. |
...
File pattern sections map glob patterns to rule packs. VectorLint reviews a file only if at least one pattern matches it.
...
Place a `VECTORLINT.md` file in your project root to define style instructions that apply to every review. VectorLint prepends its contents to the system prompt for every rule, automatically applying your tone, terminology, and baseline standards across all checks.

Keep `VECTORLINT.md` concise. VectorLint emits a warning if the file exceeds 4,000 tokens, as large context blocks degrade review performance and increase costs.
