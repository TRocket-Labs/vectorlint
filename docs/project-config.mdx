...
description: Map files to rule packs and tune review behavior with .vectorlint.ini.
---
...
You can tune how harshly individual rules score content on a per-pattern basis.
...
**Combined mode:** When `.vectorlint.ini` is present, the contents of `VECTORLINT.md` are prepended to the system prompt for every review — making your global tone and terminology preferences active across all rules.
