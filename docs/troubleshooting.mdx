...
### `--mode agent` no longer works

**Symptom:** VectorLint exits with an error that `--mode` is no longer supported.

**Cause:** Autonomous agent mode was removed. VectorLint now uses `--model-call` to choose how the reviewer model is invoked.

**Fix:** Replace the removed flag with one of the supported model-call values:
...
Use `agent` only when target-section paging helps with large or context-heavy content. It is bounded to the current target and cannot search the workspace. See [Migration from agent mode](/migration-from-agent-mode) for integration guidance.
