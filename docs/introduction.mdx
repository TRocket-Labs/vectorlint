...
VectorLint is a command-line content review harness that uses large language models (LLMs) to evaluate documentation. Instead of regex patterns that can only catch surface-level issues, VectorLint looks for observable terminology, technical accuracy, and style violations that require contextual understanding to detect.
