...
VectorLint scores each document using one of two methods, depending on what the rule evaluates:

**Density scoring** applies when a rule counts discrete issues (like a grammar checker). It calculates scores based on error density — issues per 100 words — so results are comparable across documents of any length.

**Rubric scoring** applies when a rule rates quality on a spectrum (like tone or completeness). The LLM scores each criterion on a 1–4 scale, which VectorLint normalizes to a 1–10 scale for consistent reporting.
...
VectorLint's output is intentionally stricter than raw model candidates; it reports only findings that pass all gates. You can tune how aggressively the pipeline filters findings to match your content workflow. See [Handling false positives](/handling-false-positives).
