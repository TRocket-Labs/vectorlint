...
VectorLint is a command-line tool that evaluates and scores documentation using large language models (LLMs). Unlike regex patterns that catch only surface-level issues, VectorLint can find terminology misuse, technical inaccuracies, and style inconsistencies that require contextual understanding.
