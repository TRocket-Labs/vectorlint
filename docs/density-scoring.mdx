...
description: "How VectorLint scores content when a rule counts discrete issues."
---

Density scoring is the method VectorLint uses to score a document when the rule evaluates it by counting discrete issues — banned phrases, structural errors, or missing elements. Rules define the criteria; the document is what is scored.
...
A score below **10** means the rule found at least one issue. VectorLint reports it at the rule's configured severity; a severity of `error` causes a non-zero exit code and fails CI.
...
- [Rubric scoring](/rubric-scoring) — the method used when a rule rates quality instead of counting issues
