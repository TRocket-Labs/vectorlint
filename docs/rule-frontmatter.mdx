...
| `strictness` | `standard` | For `check` rules: how harshly error density is penalized when scoring. One of `lenient` (5), `standard` (10, default), `strict` (20), or any positive number. Has no effect on `judge` rules. See [Strictness](/strictness). |
