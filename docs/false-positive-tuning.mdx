...
VectorLint gives you two complementary levers for controlling review precision. Used together, they let you calibrate how aggressively VectorLint surfaces findings — globally across your project, and per content type.

- **`CONFIDENCE_THRESHOLD`** — controls how strictly the PAT pipeline filters raw model candidates before surfacing them. A global setting that applies to every review.
- **Strictness overrides** — controls how harshly rules score error density for specific file patterns. Set per content type in `.vectorlint.ini`.
