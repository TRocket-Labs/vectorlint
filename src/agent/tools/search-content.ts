...
function searchWithRipgrep(
  ...
  for (const line of (result.stdout || '').split('\n')) {
    ...
    try {
      ...
      // Ignore parse errors in non-match lines.
    }
  }
  ...
  for (const file of files) {
    ...
    try {
      ...
      for (let i = 0; i < fileLines.length; i++) {
        if (matchCount >= limit) break;
        const current = fileLines[i] ?? '';
        if (regex.test(current)) {
          const relativeFile = path.relative(searchRoot, file);
          lines.push(`${relativeFile}:${i + 1}: ${current}`);
          matchCount++;
        }
      }
    } catch {
...
export function createSearchContentTool(cwd: string): SearchContentTool {
  return {
    name: 'search_content',
    description:
      'Search file contents for a pattern. Returns file:line: matchedtext format. Default glob filter: *.md. Supports regex patterns.',
